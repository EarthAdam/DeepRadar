...
public class DepthChart2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        bidLineRenderer = GameObject.Find("MakerLine2").GetComponent<LineRenderer>();
        bidLineRenderer.useWorldSpace = false;
        askLineRenderer = GameObject.Find("TakerLine2").GetComponent<LineRenderer>();
    ...
    IEnumerator GetText()
    {
        UnityWebRequest www = UnityWebRequest.Get("https://api.radarrelay.com/0x/v0/orderBook?baseTokenAddress=0xe94327d07fc17907b4db788e5adf2ed424addff6&quoteTokenAddress=0x2956356cd2a2bf3202f771f50d3d14a367b48070");
        ...
        else
        {
            ...
            bidLineRenderer.SetPosition(0, new Vector3(0, 0, -3));
            askLineRenderer.SetPosition(0, new Vector3(0, 0, -3));
            for (int i = 0; i < bidList.Length; i++)
            {
                bidFront += bidMakerArray[i] / 1000000000000000000;
                bidLineRenderer.SetPosition(i + 1, new Vector3(-bidMakerArray[i] / bidTakerArray[i] * 100000, bidFront, -3));
            }
            for (int i = 0; i < askList.Length; i++)
            {
                askFront += askMakerArray[i] / 10000000000000000000;
                askLineRenderer.SetPosition(i + 1, new Vector3(askMakerArray[i] / askTakerArray[i] / 5, askFront / 30, -10));
            }
            bidLineRenderer.SetPosition(bidList.Length + 1, new Vector3(-bidMakerArray[bidList.Length - 1] / bidTakerArray[bidList.Length - 1] * 100000, 0, -3));
            askLineRenderer.SetPosition(askList.Length + 1, new Vector3(askMakerArray[askList.Length - 1] / askTakerArray[askList.Length - 1] / 5, 0, -3));
            bidLineRenderer.SetPosition(bidList.Length + 2, new Vector3(0, 0, -3));
            askLineRenderer.SetPosition(askList.Length + 2, new Vector3(0, 0, -3));
            //
            /*
